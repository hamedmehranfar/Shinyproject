...
if (input$bridge_type == "ALL") {
  ...
  bar_data_2 <- components %>% count(Type, CS) %>% group_by(Type) %>% mutate(prob = (n/sum(n)*100))
  bar_plot_2<- plot_ly(bar_data_2, x = ~Type, y = ~prob , color = ~CS ,type = "bar",
                     text = ~prob %>% round() ) %>% layout(yaxis = list(ticksuffix = "%") ) 
  
} else {
  
  bar_data_2 <- components %>% filter(Type == bridge_typ) %>% count(Type, CS) %>% group_by(Type) %>% mutate(prob = (n/sum(n)*100))
  bar_plot_2<- plot_ly(bar_data_2, x = ~Type, y = ~prob , color = ~CS ,type = "bar",
                     text = ~prob %>% round() ) %>% layout(yaxis = list(ticksuffix = "%") ) 
}
} )
...
```{r print_input, eval=FALSE, include=FALSE}
renderPrint(input$bridge_type)
```
